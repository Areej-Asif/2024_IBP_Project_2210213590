        ...
        <form action="{{ route("admin.tickets.update", [$ticket->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            @else
                <div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
                    <label for="category">{{ trans('cruds.ticket.fields.category') }}*</label>
                    <select name="category_id" id="category" class="form-control select2" required>
                        <option value="{{ $ticket->category->id }}" seelcted>{{ $categories[$ticket->category->id] }}</option>
                    </select>
                    @if($errors->has('category_id'))
                        <em class="invalid-feedback">
                            {{ $errors->first('category_id') }}
            ...
            @endif
            <div>
                {{-- <input type="hidden" value="{{auth()->user()->id}}" name='assigned_to_user_id'/> --}}
                {{-- <input type="hidden" id="author_name" name="author_id" class="form-control" value="{{auth()->user()->id}}"> --}}
                {{-- <input type="hidden" id="author_email" name="author_email" class="form-control" value="{{$ticket->author_email }}"> --}}
