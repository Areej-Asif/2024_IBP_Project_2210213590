        ...
        <form action="{{ route("admin.tickets.store") }}" method="POST" enctype="multipart/form-data">
            ...
            @if(auth()->user()->isAdmin())
                <div class="form-group {{ $errors->has('assigned_to_user_id') ? 'has-error' : '' }}">
                    <label for="assigned_to_user">Assign To Department</label>
            ...
            @else
                <input type="hidden" id="author_id" name="author_id" class="form-control" value="{{auth()->user()->id}}">
                {{-- <input type="hidden" id="author_email" name="author_email" class="form-control" value="{{auth()->user()->email }}"> --}}
